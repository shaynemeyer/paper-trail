...
- Missing `created_at` / `updated_at` on a table model — every table needs both, as
  `datetime` fields with `sa_column=Column(DateTime(timezone=True), nullable=False)`
  (not naive `TIMESTAMP WITHOUT TIME ZONE` — asyncpg rejects tz-aware values against
  a naive column)
- `updated_at` not bumped (`db_obj.updated_at = datetime.now(UTC)`) in every
  PUT/PATCH route before commit — it's not a DB-level trigger, so routes must set it
  explicitly
